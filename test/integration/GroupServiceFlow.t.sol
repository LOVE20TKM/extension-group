...
contract GroupServiceFlowTest is BaseGroupFlowTest {
    ...
    function test_full_group_service_flow() public {
        ...
        alice.groupServiceAddress = h.group_service_create(
            alice,
            h.firstTokenAddress()
        );
        alice.groupServiceActionId = h.submit_group_service_action(alice);
        alice.flow.actionId = alice.groupServiceActionId;
        h.vote(alice.flow);
        ...
        // Bob joins alice's service (bob has active group)
        bob.groupServiceAddress = alice.groupServiceAddress;
        bob.groupServiceActionId = alice.groupServiceActionId;
        ...
        // Core verify for group action (bob verifies)
        ...
        // Group service provider (bob) claims from alice's service
    ...
    function _setServiceRecipients() internal {
        ...
        bob.recipients = recipients;
        bob.basisPoints = basisPoints;
        h.group_service_set_recipients(bob);
    }

    function _submitGroupScoreForService() internal {
        address[] memory membersToScore = new address[](1);
        membersToScore[0] = member1.userAddress;
        uint256[] memory scores = new uint256[](1);
        scores[0] = 100;
        h.group_submit_score(bob, membersToScore, scores);
    }
    ...
    function _coreVerifyService() internal {
        h.core_verify_extension(
            alice.flow,
            h.firstTokenAddress(),
            alice.groupServiceActionId,
            alice.groupServiceAddress
        );
    }
    ...
    function _verifyServiceRewardClaim(uint256 verifyRound) internal {
        ...
        // 3. Calculate expected bob's service reward
        // bob is only provider with only group action, so he gets 100%
        uint256 bobGeneratedReward = ga.generatedRewardByVerifier(
            verifyRound,
            bob.flow.userAddress
        );
        assertEq(bobGeneratedReward, expectedGAReward, "Bob generated all");

        uint256 expectedBobTotal = (expectedServiceReward *
            bobGeneratedReward) / expectedGAReward;
    ...
    function _verifyServiceRecipientsConfig(
        ...
        (
            ...
            uint256 ownerAmt
        ) = gs.rewardDistribution(verifyRound, bob.flow.userAddress);
    ...
    function _claimAndVerifyServiceTransfers(
        ...
        // Claim
        uint256 claimed = h.group_service_claim_reward(bob, verifyRound);
        ...
        assertEq(
            IERC20(h.firstTokenAddress()).balanceOf(bob.flow.userAddress) -
                bobBal,
            expectedBob,
            "Bob transfer"
        );
    ...
    function _verifyActionRewardClaim(
        ...
        uint256 reward = h.group_action_claim_reward(m1, bob, verifyRound);
